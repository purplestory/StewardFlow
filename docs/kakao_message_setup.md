...
### 5. 메시지 클릭 시 상세 페이지로 이동(권장)

예약/반납 알림에서 바로 해당 상세 화면으로 이동하려면 템플릿에 **웹링크 버튼**을 추가하세요.

- 예시 버튼명: `상세 보기`
- 링크 타입: `웹 링크`
- 링크 URL: `#{service_url}` 또는 카카오 템플릿에서 지정 가능한 서비스 URL 변수 사용

StewardFlow는 알림 종류에 따라 `service_url`을 동적으로 구성해 다음 경로로 전달합니다.
