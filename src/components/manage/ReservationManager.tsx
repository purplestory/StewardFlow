...
export default function ReservationManager() {
  ...
  const handleStatusChange = async (
    ...
    const response = await fetch("/api/reservations/asset", {
      method: "PATCH",
                    ...
                    <div className="mt-2 flex flex-wrap items-center gap-2 text-xs">
                      {context && reservation.assets ? (
                        <span className="chip-muted">
                          승인 권한: {roleLabel[resolveRequiredRole(policies, reservation.assets)]}
                        </span>
                      ) : null}
                      {reservation.note ? (
                        <span className="chip-muted max-w-full truncate">사유: {reservation.note}</span>
                      ) : null}
                    </div>
                  </div>
...
const resolveRequiredRole = (
  ...
  asset: { owner_department?: string; owner_scope?: string }
): ProfileRole => {
